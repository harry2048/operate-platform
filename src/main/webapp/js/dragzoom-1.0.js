...
;(function($){
    $.fn.dragZoom=function(options){
        var defaults ={
            ...
            'multiple':1.10,//放大缩小倍数
            'onlyDrag':false,//只拖拽 注意如果设置true 记得dzSon和dzDad相同
            'onlyZoom':false,//只放大缩小 注意如果设置true 记得dzSon和dzDad相同
            'minChoose':true,//是否限制缩小的最小值
            'minWidth':150,//图片缩小的最小宽度,如需启用请把minChoose设为true
            'minHeight':150,//图片缩小的最小高度,如需启用请把minChoose设为true
            'maxChoose':false,//是否限制放大的最大值
            'maxWidth':800,//图片缩小的最小宽度,如需启用请把maxChoose设为true
            'maxHeight':800,//图片缩小的最小高度，如需启用请把maxChoose设为true
...
//                $(".fixImg").css("margin-top",-options.dzSon.height()/2);
//                $(".fixImg").css("margin-left",-options.dzSon.width()/2);
//            }else{
//                options.dzSon.width(options.dzSon.width()/options.multiple);
//                options.dzSon.height(options.dzSon.height()/options.multiple);
////                options.dzSon.height(options.dzSon.height()*options.multiple);
